...
module.exports = class PlugwiseThermostatDevice extends PlugwiseDevice {
    ...
    onInit(...props) {
        ...
        this._setLocationPresetAction = new Homey.FlowCardAction('Set_Location_Preset').register()
            .registerRunListener(this._setLocationPresetRunListener.bind(this));

        this._locationPresetChangedTrigger = new Homey.FlowCardTriggerDevice('location_preset:changed').register();
    }

    async _setLocationPresetRunListener(args) {
        if (!args.hasOwnProperty('preset'))
            throw new Error('preset_property_missing');

        return this.onCapabilityLocationPreset(args.preset, args.device)
    }

    async onCapabilityLocationPreset(preset, device) {
