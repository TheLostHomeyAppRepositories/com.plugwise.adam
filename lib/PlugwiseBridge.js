...
module.exports = class PlugwiseBridge extends EventEmitter {
  ...
  */
  ...
  _poll() {
    this.getDomainObjects({
      ...
      .then(({
        ...
        Array.isArray(appliances) && appliances.forEach(appliance => {
  ...
  async setPreset({ applianceId, locationId, preset }) {
    if (!locationId && applianceId) {
      locationId = await this.getApplianceLocationId({applianceId});
    }
